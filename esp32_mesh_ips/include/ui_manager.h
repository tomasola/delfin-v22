...
public:
  ...
  // Mapeo metros -> pixeles
  const float scale = 40.0; // 1m = 40px
  const int offsetX = 240;  // Centro de la pantalla
  ...
  void drawAnchor(float mx, float my, const char *id) {
    int px = offsetX + (int)(mx * scale);
    int py = offsetY - (int)(my * scale);
    _gfx->fillRoundRect(px - 15, py - 10, 30, 20, 3, C_GRAY);
    _gfx->drawRoundRect(px - 15, py - 10, 30, 20, 3, C_TEXT);
    _gfx->setTextColor(C_TEXT);
    _gfx->setTextSize(1);
    _gfx->setCursor(px - 10, py - 4);
    _gfx->print(id);
  }

  void drawUser(float mx, float my, const char *name) {
    int px = offsetX + (int)(mx * scale);
    int py = offsetY - (int)(my * scale);

    // Constrain a zona de mapa
    px = constrain(px, 20, 460);
    py = constrain(py, 60, 270);

    _gfx->fillCircle(px, py, 6, C_PRIMARY);
    _gfx->drawCircle(px, py, 10, C_PRIMARY);
    _gfx->setTextColor(C_TEXT);
    _gfx->setCursor(px + 12, py - 4);
    _gfx->print(name);
  }
